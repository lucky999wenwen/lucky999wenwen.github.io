...
$('.xin').on('click',function () { 
	$('.xin .first li').css({animation:" showxin 8s linear infinite"},function () {  
	   
	})
	$('.xin .showphoto').css({opacity:1})
...
$(".stages").on('click',function(){
	...
	if(n%2==1){
		if(n%3==0){
			$('.stages').css({"animation-play-state":"paused"})
		}
	$('.maxbox li').eq(0).css({transform:"rotateX(90deg) translateZ(-250px)"})
	$('.maxbox li').eq(1).css({transform:"rotateX(-90deg) translateZ(-250px)"})
	$('.maxbox li').eq(2).css({transform:" rotateY(-90deg) translateZ(250px)"})
	$('.maxbox li').eq(3).css({transform:" rotateY(90deg) translateZ(250px)"})
	$('.maxbox li').eq(4).css({transform:"translateZ(-250px)"})
	$('.maxbox li').eq(5).css({transform:"translateZ(250px)"})
	}
	else{
	$('.stages').css({"animation-play-state":"running"})
	$('.maxbox li').eq(0).css({transform:"rotateX(90deg) translateZ(-150px)"})
	$('.maxbox li').eq(1).css({transform:"rotateX(-90deg) translateZ(-150px)"})
	$('.maxbox li').eq(2).css({transform:" rotateY(-90deg) translateZ(150px)"})
	$('.maxbox li').eq(3).css({transform:" rotateY(90deg) translateZ(150px)"})
	$('.maxbox li').eq(4).css({transform:"translateZ(-150px)"})
	$('.maxbox li').eq(5).css({transform:"translateZ(150px)"})
	}
})